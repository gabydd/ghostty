...
const inspect = @import("inspector/main.zig");
...
/// The render state
renderer_state: render.State,

/// The renderer thread manager
renderer_thread: render.Thread,
...
/// Terminal inspector
inspector: ?*inspect.Inspector = null,
...
const DerivedConfig = struct {
    ...
    fn scaledPadding(self: *const DerivedConfig, x_dpi: f32, y_dpi: f32) render.Padding {
...
pub fn init(
    ...
    const size: render.Size = size: {
        ...
        const explicit: render.Padding = derived_config.scaledPadding(
            x_dpi,
            y_dpi,
        );
    ...
    var render_thread = try render.Thread.init(
        alloc,
        config,
        rt_surface,
        &self.renderer,
...
pub fn activateInspector(self: *Surface) !void {
    ...
    // Setup the inspector
    const ptr = try self.alloc.create(inspect.Inspector);
    errdefer self.alloc.destroy(ptr);
    ptr.* = try inspect.Inspector.init(self);
    self.inspector = ptr;
...
fn updateRendererHealth(self: *Surface, health: render.Health) void {
...
pub fn updateConfig(
    ...
    var renderer_message = try render.Message.initChangeConfig(self.alloc, config);
...
fn setCellSize(self: *Surface, size: render.CellSize) !void {
...
pub fn sizeCallback(self: *Surface, size: apprt.SurfaceSize) !void {
    ...
    const new_screen_size: render.ScreenSize = .{
        .width = size.width,
        .height = size.height,
    };
...
fn resize(self: *Surface, size: render.ScreenSize) !void {
...
pub fn preeditCallback(self: *Surface, preedit_: ?[]const u8) !void {
    ...
    const Codepoint = render.State.Preedit.Codepoint;
...
pub fn keyCallback(
    ...
    var insp_ev: ?inspect.key.Event = if (self.inspector != null) ev: {
        var copy = event;
        copy.utf8 = "";
...
fn mouseReport(
    ...
    switch (self.io.terminal.flags.mouse_format) {
        ...
        .sgr_pixels => {
            ...
            const coord: render.Coordinate.Terminal = (render.Coordinate{
                .surface = .{
                    .x = pos.x,
                    .y = pos.y,
                },
...
pub fn posToViewport(self: Surface, xpos: f64, ypos: f64) terminal.point.Coordinate {
    // Get our grid cell
    const coord: render.Coordinate = .{ .surface = .{ .x = xpos, .y = ypos } };
