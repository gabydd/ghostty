...
pub const Mailbox = BlockingQueue(render.Message, 64);
...
fn threadMain_(self: *Thread) !void {
    ...
    const has_loop = @hasDecl(render.Renderer, "loopEnter");
...
fn startDrawTimer(self: *Thread) void {
    ...
    if (!@hasDecl(render.Renderer, "hasAnimations")) return;
...
fn drainMailbox(self: *Thread) !void {
    while (self.mailbox.pop()) |message| {
        switch (message) {
            ...
            .macos_display_id => |v| {
                if (@hasDecl(render.Renderer, "setMacOSDisplayID")) {
...
fn drawFrame(self: *Thread, now: bool) void {
    ...
    if (render.Renderer == render.OpenGL and
        render.OpenGL.single_threaded_draw)
    {
